...
@SqlGroup({
        @Sql(scripts = "/sql/techinfo/default-insert-like-data.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD),
        @Sql(scripts = "/sql/techinfo/default-delete-all-data.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
})
@SpringBootTest
@ActiveProfiles("test")
class LikeQueryServiceImplTest {
    ...
    void existsByUserNicknameAndArticleId(final String userNickname, final String articleId) {
        // When
        final boolean result = likeQueryService.existsByUserNicknameAndArticleId(userNickname, articleId);

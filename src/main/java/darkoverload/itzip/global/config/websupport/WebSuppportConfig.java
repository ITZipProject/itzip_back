...
public class WebSuppportConfig implements WebMvcConfigurer {
    ...
    /**
     * PagedResourcesAssembler를 빈으로 등록하는 메서드
     * PagedResourcesAssembler는 페이징 처리된 데이터에 HATEOAS 링크를 추가하는데 사용
     * 이 빈은 customPagedResourcesAssembler라는 이름으로 등록
     *
    ...
    /**
     * Spring MVC의 Argument Resolver 목록에 PagedResourcesAssemblerArgumentResolver를 추가하는 메서드
     * 이 Resolver는 컨트롤러 메서드에서 PagedResourcesAssembler를 사용 가능하게 함.
     * 기본 페이징 및 정렬 설정을 처리하고, 페이징된 리소스에 링크를 추가하는 역할을 함.
     *

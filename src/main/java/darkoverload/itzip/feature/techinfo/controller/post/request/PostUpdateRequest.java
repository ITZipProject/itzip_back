...
@Schema(
        description = "포스트 업데이트 요청 객체로, 수정할 포스트의 ID와 수정된 내용을 포함합니다."
)
@Getter
@NoArgsConstructor
@AllArgsConstructor
public class PostUpdateRequest {
    ...
    @Schema(description = "수정할 포스트의 ID", example = "66e724e50000000000db4e53")
    ...
    @Schema(description = "수정된 포스트 제목", example = "밤하늘 아래, 감정의 여정")
    @NotBlank(message = "포스트 제목은 필수 입력 항목입니다.")
    private String title;

    @Schema(
            description = "수정된 포스트 본문",
            example = """
                이 세 개의 이미지는 감정의 복잡한 여정을 시각적으로 표현하고 있다. 첫 번째 장면에서는 소녀가 꿈을 향해 하늘로 비상하려 하지만, 현실의 무게에 의해 아래로 끌려 내려가는 모습을 담고 있다. 
                이는 꿈과 야망을 추구하는 과정에서 마주하는 좌절과 도전을 상징한다. 두 번째 장면에서는 소녀가 밤하늘을 응시하며, 자신의 과거와 잊지 못한 꿈들을 회상하는 장면이 그려진다. 
                창문 밖으로 보이는 거친 파도는 그녀의 내면에서 일어나는 감정의 소용돌이를 나타낸다. 마지막 장면은 비가 내리는 고요한 거리에서 두 사람이 나란히 걷는 모습을 통해, 서로에게 의지하며 고난을 함께 이겨내는 모습을 보여준다. 
                이 이미지들은 꿈, 좌절, 그리고 관계 속에서 위로를 찾는 여정을 이야기하며, 인간이 감정을 어떻게 극복하고 성장하는지에 대한 메시지를 전달한다.
                """
    )
    ...
    @Schema(
            description = "수정된 포스트 본문 이미지 경로 리스트",
            example = """
                  [

...
public class CustomPostRepositoryImpl implements CustomPostRepository {
    ...
    public Page<PostDocument> findPostsByBlogId(Long blogId, Pageable pageable) {
        Criteria criteria = Criteria.where("blog_id").is(blogId).and("is_public").is(true); // 블로그 ID와 공개 여부 조건 추가
        return executeQueryWithCriteria(criteria, pageable, false); // 실행
    }

    // 이전/이후 포스트를 조회하는 메서드
    @Override
    public List<PostDocument> findAdjacentPosts(Long blogId, LocalDateTime createDate, int limit) {
    ...
    @Override
    public List<YearlyPostDto> findYearlyPostData(Long blogId) {
